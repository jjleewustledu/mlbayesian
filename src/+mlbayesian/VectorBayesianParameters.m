classdef VectorBayesianParameters < mlbayesian.IBayesianParameters
	%% VECTORBAYESIANPARAMETERS 
    ...
    methods 
        function this = VectorBayesianParameters(pmap)
            %% VECTORBAYESIANPARAMETERS 
            %  Usage:  pmap = containers.Map
            %          for p = 1:LengthPmap
            %              pmap(p) = struct('fixed', 0, 'min', eps, 'mean', 1, 'max',  10)
            %          end
            %          this = VectorBayesianParameters(pmap)
    ...
    methods (Access = 'protected')  
        function this = buildParamsIndices(this)
            ...
            this.paramsIndices = containers.Map('KeyType', 'double', 'ValueType', 'any');
            for k = 1:length(keys)
